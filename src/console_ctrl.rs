...
//! the process and, absent a handler, simply tears it down. That was enough to
//! leave a radio station playing — the shutdown never ran, and the process
//! could sit wedged in its runtime teardown with the audio device still open
//! long after the window was gone.
//!
...
/// close-button path, which is what it was added to improve.
pub fn install() {
    unsafe {
...
unsafe extern "system" fn handler(event: u32) -> BOOL {
    ...
    // process; the audio device is the one thing that was outliving it.
    ...
    // termination is exactly what was not finishing — and Ctrl-C is not a close
